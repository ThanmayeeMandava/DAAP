...
def load_data_to_mongodb() -> bool:
    ...
    try:
        ...
        # Continue fetching data until there is no data left to fetch
        while True:
            ...
            if isinstance(data, list) and len(data) > 0:
                ...
                offset += len(data)  # Move the offset
                logger.info(f"Fetched and inserted {len(data)} records successfully.")
            else:
                # If no data is returned, we're done
                break

        logger.info("All data successfully fetched from the website and inserted into MongoDB.")
        result = True
...
def transform_data(df) -> pd.DataFrame:
    ...
    #----------------------------------------------------------------------------------------
        ...
        logger.info("Transformation 1: Standardized state names.")

    #----------------------------------------------------------------------------------------

        # Transformation 2: 
        # Convert data types
        numeric_columns = ['deaths', 'population', 'crude_death_rate', 
                           'age_adjusted_rate', 'us_crude_rate', 'us_age_adjusted_rate']
        ...
        for col in numeric_columns:
            ...
            logger.info(f"Transformation 2: Converted {col} to numeric, replaced {original_non_numeric} non-numeric values.")
    ...
    #----------------------------------------------------------------------------------------
        ...
        if df['age_adjusted_rate'].isnull().any():
            ...
            logger.info("Transformation 3: Filled missing values in 'age_adjusted_rate'.")
    ...
    #----------------------------------------------------------------------------------------
        ...
        df[numeric_cols.columns] = numeric_cols.round(2)
        logger.info("Transformation 4: Rounded off the numeric columns to 2 decimel places.")
    ...
    #----------------------------------------------------------------------------------------
        ...
        logger.info("Transformation 5: Clipped negative values.")
    ...
    #----------------------------------------------------------------------------------------
        ...
        logger.info("Transformation 6: Filtered records by year range.")
...
#----------------------------------------------------------------------------------------
        ...
        logger.info(f"Transformation 7: Number of duplicate rows: {duplicate_count}")
        
        # Dropping duplicate rows if found.
        if(duplicate_count):
            df = df.drop_duplicates()
            logger.info("Transformation 7: Dropped duplicate rows.")
...
# Creating function to initialize PostgreSQL session with credentials and Re-use credentials.
def init_pg_session():
...
# Creating SQLAlchemy Table Model by selecting only required columns, i.e dropping irrelevant columns.
class NCHSData(Base):
